...
האפליקציה בנויה כ-PWA (Progressive Web App) עם אייקון, מסך פתיחה ומצב עבודה
עצמאי (בלי סרגלי דפדפן) — כלומר אפשר "להתקין" אותה כמו אפליקציה רגילה,
בלי חנות אפליקציות:

**אייפון (Safari):** פותחים את הקישור ב-Safari → כפתור השיתוף (הריבוע עם
החץ) → **"הוספה למסך הבית"**.

**אנדרואיד (Chrome):** פותחים את הקישור → תפריט שלוש הנקודות → **"התקנת
אפליקציה"** (או באנר שיופיע אוטומטית בתחתית המסך).
